<template>
  <div>
    <div class="card">
      <label for="months">Выберите месяц:</label>
      <select id="months" v-model="selectedMonth">
        <option v-for="subject in months" :key="subject" :value="subject">
          {{ subject }}
        </option>
      </select>
      <label for="subject">Выберите предмет:</label>
      <select id="subject" v-model="selectedSubject">
        <option v-for="subject in subjects" :key="subject" :value="subject">
          {{ subject }}
        </option>
      </select>
    </div>
    <div v-if="grtadeesd" v-for="grtadees in grtadeesd">
      <div class="card">Общий балл: {{ grtadees.grades.month_avg }}</div>
      <div class="container">
        <div class="card_grades">
          <div class="date_grades">Дата</div>
          <div class="assessment">Оценка</div>
        </div>
        <div class="card_grades" v-for="grades in grtadees.grades.dates">
          <div class="date_grades">
            {{
              new Date(grades[0]).toLocaleDateString("ru-RU", {
                year: "numeric",
                month: "long",
                day: "numeric",
              })
            }}
          </div>
          <div class="assessment">{{ grades[1] }}</div>
        </div>
      </div>
    </div>
    <div v-else>
      <div class="card" style="align-items: center">
        Выберите месяц и предмет
      </div>
    </div>
  </div>
</template>

<style scoped>
select {
  appearance: none;
  background-color: transparent;
  border: none;
  padding: 0 1em 0 0;
  margin: 0;
  width: 100%;
  font-family: inherit;
  font-size: inherit;
  cursor: inherit;
  line-height: inherit;
  border: 1px solid black;
  border-radius: 5px;
  text-align: center;
}

.card {
  margin-top: 20px;
  flex-direction: column;
}

.card_grades {
  width: 90%;
  border-bottom: 1px solid gainsboro;
  height: 30px;
  padding: 10px;
  display: flex;
  flex-direction: row;
  justify-content: space-between;
}
.container {
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
}
</style>

<script>
import { ref, watch } from "vue";
const jsonData = [
  {
    subject: "Введение в программирование",
    month: "Январь",
    grades: {
      subject: "Введение в программирование",
      month_avg: "4.04",
      dates: [
        ["2023-01-01", 5],
        ["2023-01-02", 4],
        ["2023-01-03", 4],
        ["2023-01-04", 5],
        ["2023-01-05", 4],
        ["2023-01-06", 5],
        ["2023-01-07", 4],
        ["2023-01-08", 4],
        ["2023-01-09", 3],
        ["2023-01-10", 4],
        ["2023-01-11", 5],
        ["2023-01-12", 4],
        ["2023-01-13", 5],
        ["2023-01-14", 5],
        ["2023-01-15", 3],
        ["2023-01-16", 4],
        ["2023-01-17", 3],
        ["2023-01-18", 3],
        ["2023-01-19", 4],
        ["2023-01-20", 4],
        ["2023-01-21", 5],
        ["2023-01-22", 3],
        ["2023-01-23", 3],
        ["2023-01-24", 4],
        ["2023-01-25", 3],
        ["2023-01-26", 5],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Февраль",
    grades: {
      subject: "Введение в программирование",
      month_avg: "3.95",
      dates: [
        ["2023-02-01", 4],
        ["2023-02-02", 5],
        ["2023-02-03", 3],
        ["2023-02-04", 5],
        ["2023-02-05", 3],
        ["2023-02-06", 5],
        ["2023-02-07", 4],
        ["2023-02-08", 4],
        ["2023-02-09", 4],
        ["2023-02-10", 4],
        ["2023-02-11", 4],
        ["2023-02-12", 3],
        ["2023-02-13", 5],
        ["2023-02-14", 3],
        ["2023-02-15", 5],
        ["2023-02-16", 4],
        ["2023-02-17", 3],
        ["2023-02-18", 4],
        ["2023-02-19", 3],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Март",
    grades: {
      subject: "Введение в программирование",
      month_avg: "3.86",
      dates: [
        ["2023-03-01", 3],
        ["2023-03-02", 5],
        ["2023-03-03", 3],
        ["2023-03-04", 5],
        ["2023-03-05", 4],
        ["2023-03-06", 4],
        ["2023-03-07", 4],
        ["2023-03-08", 4],
        ["2023-03-09", 4],
        ["2023-03-10", 4],
        ["2023-03-11", 3],
        ["2023-03-12", 3],
        ["2023-03-13", 3],
        ["2023-03-14", 5],
        ["2023-03-15", 5],
        ["2023-03-16", 4],
        ["2023-03-17", 4],
        ["2023-03-18", 4],
        ["2023-03-19", 3],
        ["2023-03-20", 3],
        ["2023-03-21", 4],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Апрель",
    grades: {
      subject: "Введение в программирование",
      month_avg: "4.00",
      dates: [
        ["2023-04-01", 4],
        ["2023-04-02", 5],
        ["2023-04-03", 4],
        ["2023-04-04", 5],
        ["2023-04-05", 3],
        ["2023-04-06", 5],
        ["2023-04-07", 4],
        ["2023-04-08", 4],
        ["2023-04-09", 3],
        ["2023-04-10", 4],
        ["2023-04-11", 4],
        ["2023-04-12", 4],
        ["2023-04-13", 4],
        ["2023-04-14", 3],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Май",
    grades: {
      subject: "Введение в программирование",
      month_avg: "4.50",
      dates: [
        ["2023-05-01", 5],
        ["2023-05-02", 5],
        ["2023-05-03", 5],
        ["2023-05-04", 4],
        ["2023-05-05", 5],
        ["2023-05-06", 3],
        ["2023-05-07", 4],
        ["2023-05-08", 5],
        ["2023-05-09", 4],
        ["2023-05-10", 5],
        ["2023-05-11", 5],
        ["2023-05-12", 4],
        ["2023-05-13", 4],
        ["2023-05-14", 5],
        ["2023-05-15", 4],
        ["2023-05-16", 5],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Июнь",
    grades: {
      subject: "Введение в программирование",
      month_avg: "3.76",
      dates: [
        ["2023-06-01", 4],
        ["2023-06-02", 5],
        ["2023-06-03", 3],
        ["2023-06-04", 4],
        ["2023-06-05", 5],
        ["2023-06-06", 4],
        ["2023-06-07", 5],
        ["2023-06-08", 4],
        ["2023-06-09", 3],
        ["2023-06-10", 3],
        ["2023-06-11", 3],
        ["2023-06-12", 5],
        ["2023-06-13", 3],
        ["2023-06-14", 4],
        ["2023-06-15", 5],
        ["2023-06-16", 3],
        ["2023-06-17", 3],
        ["2023-06-18", 3],
        ["2023-06-19", 3],
        ["2023-06-20", 5],
        ["2023-06-21", 3],
        ["2023-06-22", 3],
        ["2023-06-23", 3],
        ["2023-06-24", 4],
        ["2023-06-25", 4],
        ["2023-06-26", 3],
        ["2023-06-27", 4],
        ["2023-06-28", 4],
        ["2023-06-29", 4],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Сентябрь",
    grades: {
      subject: "Введение в программирование",
      month_avg: "3.81",
      dates: [
        ["2023-09-01", 5],
        ["2023-09-02", 3],
        ["2023-09-03", 4],
        ["2023-09-04", 4],
        ["2023-09-05", 4],
        ["2023-09-06", 5],
        ["2023-09-07", 3],
        ["2023-09-08", 4],
        ["2023-09-09", 5],
        ["2023-09-10", 4],
        ["2023-09-11", 3],
        ["2023-09-12", 3],
        ["2023-09-13", 3],
        ["2023-09-14", 5],
        ["2023-09-15", 3],
        ["2023-09-16", 3],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Октябрь",
    grades: {
      subject: "Введение в программирование",
      month_avg: "4.26",
      dates: [
        ["2023-10-01", 5],
        ["2023-10-02", 5],
        ["2023-10-03", 3],
        ["2023-10-04", 5],
        ["2023-10-05", 5],
        ["2023-10-06", 4],
        ["2023-10-07", 5],
        ["2023-10-08", 4],
        ["2023-10-09", 5],
        ["2023-10-10", 4],
        ["2023-10-11", 3],
        ["2023-10-12", 4],
        ["2023-10-13", 5],
        ["2023-10-14", 3],
        ["2023-10-15", 3],
        ["2023-10-16", 3],
        ["2023-10-17", 4],
        ["2023-10-18", 5],
        ["2023-10-19", 4],
        ["2023-10-20", 5],
        ["2023-10-21", 4],
        ["2023-10-22", 4],
        ["2023-10-23", 5],
        ["2023-10-24", 4],
        ["2023-10-25", 4],
        ["2023-10-26", 5],
        ["2023-10-27", 5],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Ноябрь",
    grades: {
      subject: "Введение в программирование",
      month_avg: "4.11",
      dates: [
        ["2023-11-01", 4],
        ["2023-11-02", 5],
        ["2023-11-03", 5],
        ["2023-11-04", 3],
        ["2023-11-05", 4],
        ["2023-11-06", 4],
        ["2023-11-07", 3],
        ["2023-11-08", 5],
        ["2023-11-09", 3],
        ["2023-11-10", 5],
        ["2023-11-11", 5],
        ["2023-11-12", 4],
        ["2023-11-13", 5],
        ["2023-11-14", 3],
        ["2023-11-15", 5],
        ["2023-11-16", 3],
        ["2023-11-17", 5],
        ["2023-11-18", 3],
      ],
    },
  },
  {
    subject: "Введение в программирование",
    month: "Декабрь",
    grades: {
      subject: "Введение в программирование",
      month_avg: "3.77",
      dates: [
        ["2023-12-01", 3],
        ["2023-12-02", 5],
        ["2023-12-03", 3],
        ["2023-12-04", 5],
        ["2023-12-05", 3],
        ["2023-12-06", 5],
        ["2023-12-07", 3],
        ["2023-12-08", 3],
        ["2023-12-09", 3],
        ["2023-12-10", 4],
        ["2023-12-11", 4],
        ["2023-12-12", 4],
        ["2023-12-13", 4],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Январь",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "4.06",
      dates: [
        ["2023-01-01", 4],
        ["2023-01-02", 5],
        ["2023-01-03", 5],
        ["2023-01-04", 4],
        ["2023-01-05", 3],
        ["2023-01-06", 3],
        ["2023-01-07", 4],
        ["2023-01-08", 5],
        ["2023-01-09", 3],
        ["2023-01-10", 4],
        ["2023-01-11", 3],
        ["2023-01-12", 5],
        ["2023-01-13", 3],
        ["2023-01-14", 5],
        ["2023-01-15", 5],
        ["2023-01-16", 4],
        ["2023-01-17", 4],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Февраль",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "4.07",
      dates: [
        ["2023-02-01", 5],
        ["2023-02-02", 3],
        ["2023-02-03", 3],
        ["2023-02-04", 4],
        ["2023-02-05", 3],
        ["2023-02-06", 3],
        ["2023-02-07", 3],
        ["2023-02-08", 5],
        ["2023-02-09", 3],
        ["2023-02-10", 5],
        ["2023-02-11", 3],
        ["2023-02-12", 4],
        ["2023-02-13", 5],
        ["2023-02-14", 5],
        ["2023-02-15", 5],
        ["2023-02-16", 4],
        ["2023-02-17", 5],
        ["2023-02-18", 5],
        ["2023-02-19", 3],
        ["2023-02-20", 5],
        ["2023-02-21", 4],
        ["2023-02-22", 4],
        ["2023-02-23", 5],
        ["2023-02-24", 5],
        ["2023-02-25", 3],
        ["2023-02-26", 5],
        ["2023-02-27", 5],
        ["2023-02-28", 4],
        ["2023-03-01", 3],
        ["2023-03-02", 3],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Март",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "3.81",
      dates: [
        ["2023-03-01", 3],
        ["2023-03-02", 5],
        ["2023-03-03", 3],
        ["2023-03-04", 4],
        ["2023-03-05", 5],
        ["2023-03-06", 3],
        ["2023-03-07", 4],
        ["2023-03-08", 3],
        ["2023-03-09", 4],
        ["2023-03-10", 3],
        ["2023-03-11", 3],
        ["2023-03-12", 3],
        ["2023-03-13", 4],
        ["2023-03-14", 5],
        ["2023-03-15", 5],
        ["2023-03-16", 4],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Апрель",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "4.23",
      dates: [
        ["2023-04-01", 5],
        ["2023-04-02", 5],
        ["2023-04-03", 5],
        ["2023-04-04", 3],
        ["2023-04-05", 5],
        ["2023-04-06", 4],
        ["2023-04-07", 5],
        ["2023-04-08", 3],
        ["2023-04-09", 5],
        ["2023-04-10", 4],
        ["2023-04-11", 3],
        ["2023-04-12", 3],
        ["2023-04-13", 5],
        ["2023-04-14", 5],
        ["2023-04-15", 5],
        ["2023-04-16", 5],
        ["2023-04-17", 4],
        ["2023-04-18", 3],
        ["2023-04-19", 3],
        ["2023-04-20", 5],
        ["2023-04-21", 4],
        ["2023-04-22", 4],
        ["2023-04-23", 5],
        ["2023-04-24", 4],
        ["2023-04-25", 4],
        ["2023-04-26", 4],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Май",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "4.19",
      dates: [
        ["2023-05-01", 3],
        ["2023-05-02", 5],
        ["2023-05-03", 3],
        ["2023-05-04", 5],
        ["2023-05-05", 4],
        ["2023-05-06", 4],
        ["2023-05-07", 5],
        ["2023-05-08", 3],
        ["2023-05-09", 5],
        ["2023-05-10", 4],
        ["2023-05-11", 3],
        ["2023-05-12", 5],
        ["2023-05-13", 5],
        ["2023-05-14", 5],
        ["2023-05-15", 3],
        ["2023-05-16", 3],
        ["2023-05-17", 5],
        ["2023-05-18", 5],
        ["2023-05-19", 5],
        ["2023-05-20", 5],
        ["2023-05-21", 3],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Июнь",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "3.93",
      dates: [
        ["2023-06-01", 5],
        ["2023-06-02", 3],
        ["2023-06-03", 3],
        ["2023-06-04", 3],
        ["2023-06-05", 3],
        ["2023-06-06", 4],
        ["2023-06-07", 3],
        ["2023-06-08", 5],
        ["2023-06-09", 3],
        ["2023-06-10", 5],
        ["2023-06-11", 5],
        ["2023-06-12", 5],
        ["2023-06-13", 5],
        ["2023-06-14", 3],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Сентябрь",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "3.90",
      dates: [
        ["2023-09-01", 4],
        ["2023-09-02", 5],
        ["2023-09-03", 4],
        ["2023-09-04", 5],
        ["2023-09-05", 5],
        ["2023-09-06", 3],
        ["2023-09-07", 3],
        ["2023-09-08", 4],
        ["2023-09-09", 5],
        ["2023-09-10", 3],
        ["2023-09-11", 3],
        ["2023-09-12", 5],
        ["2023-09-13", 5],
        ["2023-09-14", 4],
        ["2023-09-15", 3],
        ["2023-09-16", 3],
        ["2023-09-17", 4],
        ["2023-09-18", 3],
        ["2023-09-19", 3],
        ["2023-09-20", 4],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Октябрь",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "3.92",
      dates: [
        ["2023-10-01", 3],
        ["2023-10-02", 3],
        ["2023-10-03", 3],
        ["2023-10-04", 5],
        ["2023-10-05", 4],
        ["2023-10-06", 5],
        ["2023-10-07", 5],
        ["2023-10-08", 3],
        ["2023-10-09", 3],
        ["2023-10-10", 4],
        ["2023-10-11", 4],
        ["2023-10-12", 5],
        ["2023-10-13", 4],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Ноябрь",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "4.05",
      dates: [
        ["2023-11-01", 5],
        ["2023-11-02", 5],
        ["2023-11-03", 3],
        ["2023-11-04", 3],
        ["2023-11-05", 3],
        ["2023-11-06", 5],
        ["2023-11-07", 4],
        ["2023-11-08", 4],
        ["2023-11-09", 4],
        ["2023-11-10", 5],
        ["2023-11-11", 3],
        ["2023-11-12", 3],
        ["2023-11-13", 5],
        ["2023-11-14", 3],
        ["2023-11-15", 5],
        ["2023-11-16", 5],
        ["2023-11-17", 4],
        ["2023-11-18", 3],
        ["2023-11-19", 5],
      ],
    },
  },
  {
    subject: "Основы алгоритмизации",
    month: "Декабрь",
    grades: {
      subject: "Основы алгоритмизации",
      month_avg: "3.97",
      dates: [
        ["2023-12-01", 3],
        ["2023-12-02", 4],
        ["2023-12-03", 4],
        ["2023-12-04", 3],
        ["2023-12-05", 3],
        ["2023-12-06", 4],
        ["2023-12-07", 5],
        ["2023-12-08", 3],
        ["2023-12-09", 5],
        ["2023-12-10", 3],
        ["2023-12-11", 3],
        ["2023-12-12", 3],
        ["2023-12-13", 3],
        ["2023-12-14", 5],
        ["2023-12-15", 4],
        ["2023-12-16", 5],
        ["2023-12-17", 5],
        ["2023-12-18", 4],
        ["2023-12-19", 5],
        ["2023-12-20", 3],
        ["2023-12-21", 5],
        ["2023-12-22", 5],
        ["2023-12-23", 3],
        ["2023-12-24", 4],
        ["2023-12-25", 5],
        ["2023-12-26", 5],
        ["2023-12-27", 4],
        ["2023-12-28", 4],
        ["2023-12-29", 3],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Январь",
    grades: {
      subject: "Структуры данных",
      month_avg: "3.52",
      dates: [
        ["2023-01-01", 3],
        ["2023-01-02", 4],
        ["2023-01-03", 4],
        ["2023-01-04", 4],
        ["2023-01-05", 3],
        ["2023-01-06", 4],
        ["2023-01-07", 3],
        ["2023-01-08", 4],
        ["2023-01-09", 5],
        ["2023-01-10", 4],
        ["2023-01-11", 3],
        ["2023-01-12", 3],
        ["2023-01-13", 3],
        ["2023-01-14", 3],
        ["2023-01-15", 3],
        ["2023-01-16", 5],
        ["2023-01-17", 3],
        ["2023-01-18", 3],
        ["2023-01-19", 4],
        ["2023-01-20", 3],
        ["2023-01-21", 3],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Февраль",
    grades: {
      subject: "Структуры данных",
      month_avg: "4.00",
      dates: [
        ["2023-02-01", 5],
        ["2023-02-02", 3],
        ["2023-02-03", 4],
        ["2023-02-04", 3],
        ["2023-02-05", 3],
        ["2023-02-06", 5],
        ["2023-02-07", 5],
        ["2023-02-08", 5],
        ["2023-02-09", 5],
        ["2023-02-10", 4],
        ["2023-02-11", 3],
        ["2023-02-12", 3],
        ["2023-02-13", 3],
        ["2023-02-14", 4],
        ["2023-02-15", 5],
        ["2023-02-16", 4],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Март",
    grades: {
      subject: "Структуры данных",
      month_avg: "3.93",
      dates: [
        ["2023-03-01", 4],
        ["2023-03-02", 3],
        ["2023-03-03", 5],
        ["2023-03-04", 4],
        ["2023-03-05", 5],
        ["2023-03-06", 4],
        ["2023-03-07", 5],
        ["2023-03-08", 3],
        ["2023-03-09", 4],
        ["2023-03-10", 3],
        ["2023-03-11", 3],
        ["2023-03-12", 4],
        ["2023-03-13", 4],
        ["2023-03-14", 5],
        ["2023-03-15", 3],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Апрель",
    grades: {
      subject: "Структуры данных",
      month_avg: "4.17",
      dates: [
        ["2023-04-01", 3],
        ["2023-04-02", 4],
        ["2023-04-03", 4],
        ["2023-04-04", 5],
        ["2023-04-05", 3],
        ["2023-04-06", 3],
        ["2023-04-07", 4],
        ["2023-04-08", 5],
        ["2023-04-09", 5],
        ["2023-04-10", 5],
        ["2023-04-11", 4],
        ["2023-04-12", 5],
        ["2023-04-13", 4],
        ["2023-04-14", 4],
        ["2023-04-15", 5],
        ["2023-04-16", 4],
        ["2023-04-17", 3],
        ["2023-04-18", 5],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Май",
    grades: {
      subject: "Структуры данных",
      month_avg: "4.17",
      dates: [
        ["2023-05-01", 3],
        ["2023-05-02", 3],
        ["2023-05-03", 3],
        ["2023-05-04", 5],
        ["2023-05-05", 3],
        ["2023-05-06", 4],
        ["2023-05-07", 5],
        ["2023-05-08", 3],
        ["2023-05-09", 5],
        ["2023-05-10", 5],
        ["2023-05-11", 5],
        ["2023-05-12", 3],
        ["2023-05-13", 3],
        ["2023-05-14", 5],
        ["2023-05-15", 5],
        ["2023-05-16", 5],
        ["2023-05-17", 4],
        ["2023-05-18", 4],
        ["2023-05-19", 5],
        ["2023-05-20", 3],
        ["2023-05-21", 3],
        ["2023-05-22", 5],
        ["2023-05-23", 5],
        ["2023-05-24", 5],
        ["2023-05-25", 3],
        ["2023-05-26", 5],
        ["2023-05-27", 5],
        ["2023-05-28", 5],
        ["2023-05-29", 4],
        ["2023-05-30", 4],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Июнь",
    grades: {
      subject: "Структуры данных",
      month_avg: "3.81",
      dates: [
        ["2023-06-01", 5],
        ["2023-06-02", 4],
        ["2023-06-03", 5],
        ["2023-06-04", 3],
        ["2023-06-05", 3],
        ["2023-06-06", 4],
        ["2023-06-07", 3],
        ["2023-06-08", 4],
        ["2023-06-09", 4],
        ["2023-06-10", 3],
        ["2023-06-11", 5],
        ["2023-06-12", 4],
        ["2023-06-13", 3],
        ["2023-06-14", 3],
        ["2023-06-15", 5],
        ["2023-06-16", 3],
        ["2023-06-17", 4],
        ["2023-06-18", 3],
        ["2023-06-19", 3],
        ["2023-06-20", 3],
        ["2023-06-21", 4],
        ["2023-06-22", 5],
        ["2023-06-23", 4],
        ["2023-06-24", 3],
        ["2023-06-25", 5],
        ["2023-06-26", 4],
        ["2023-06-27", 4],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Сентябрь",
    grades: {
      subject: "Структуры данных",
      month_avg: "3.74",
      dates: [
        ["2023-09-01", 3],
        ["2023-09-02", 4],
        ["2023-09-03", 3],
        ["2023-09-04", 4],
        ["2023-09-05", 3],
        ["2023-09-06", 3],
        ["2023-09-07", 3],
        ["2023-09-08", 5],
        ["2023-09-09", 5],
        ["2023-09-10", 4],
        ["2023-09-11", 3],
        ["2023-09-12", 4],
        ["2023-09-13", 4],
        ["2023-09-14", 5],
        ["2023-09-15", 5],
        ["2023-09-16", 4],
        ["2023-09-17", 3],
        ["2023-09-18", 3],
        ["2023-09-19", 4],
        ["2023-09-20", 4],
        ["2023-09-21", 4],
        ["2023-09-22", 3],
        ["2023-09-23", 5],
        ["2023-09-24", 4],
        ["2023-09-25", 3],
        ["2023-09-26", 3],
        ["2023-09-27", 3],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Октябрь",
    grades: {
      subject: "Структуры данных",
      month_avg: "3.92",
      dates: [
        ["2023-10-01", 4],
        ["2023-10-02", 4],
        ["2023-10-03", 5],
        ["2023-10-04", 5],
        ["2023-10-05", 4],
        ["2023-10-06", 4],
        ["2023-10-07", 3],
        ["2023-10-08", 3],
        ["2023-10-09", 5],
        ["2023-10-10", 5],
        ["2023-10-11", 3],
        ["2023-10-12", 3],
        ["2023-10-13", 5],
        ["2023-10-14", 3],
        ["2023-10-15", 3],
        ["2023-10-16", 4],
        ["2023-10-17", 3],
        ["2023-10-18", 3],
        ["2023-10-19", 4],
        ["2023-10-20", 5],
        ["2023-10-21", 5],
        ["2023-10-22", 5],
        ["2023-10-23", 4],
        ["2023-10-24", 3],
        ["2023-10-25", 3],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Ноябрь",
    grades: {
      subject: "Структуры данных",
      month_avg: "4.19",
      dates: [
        ["2023-11-01", 4],
        ["2023-11-02", 3],
        ["2023-11-03", 4],
        ["2023-11-04", 5],
        ["2023-11-05", 4],
        ["2023-11-06", 5],
        ["2023-11-07", 5],
        ["2023-11-08", 5],
        ["2023-11-09", 5],
        ["2023-11-10", 3],
        ["2023-11-11", 5],
        ["2023-11-12", 4],
        ["2023-11-13", 4],
        ["2023-11-14", 3],
        ["2023-11-15", 3],
        ["2023-11-16", 5],
      ],
    },
  },
  {
    subject: "Структуры данных",
    month: "Декабрь",
    grades: {
      subject: "Структуры данных",
      month_avg: "4.17",
      dates: [
        ["2023-12-01", 4],
        ["2023-12-02", 5],
        ["2023-12-03", 4],
        ["2023-12-04", 4],
        ["2023-12-05", 3],
        ["2023-12-06", 5],
        ["2023-12-07", 5],
        ["2023-12-08", 5],
        ["2023-12-09", 5],
        ["2023-12-10", 5],
        ["2023-12-11", 3],
        ["2023-12-12", 5],
        ["2023-12-13", 3],
        ["2023-12-14", 4],
        ["2023-12-15", 4],
        ["2023-12-16", 4],
        ["2023-12-17", 5],
        ["2023-12-18", 3],
        ["2023-12-19", 4],
        ["2023-12-20", 4],
        ["2023-12-21", 4],
        ["2023-12-22", 3],
        ["2023-12-23", 4],
        ["2023-12-24", 5],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Январь",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "3.63",
      dates: [
        ["2023-01-01", 3],
        ["2023-01-02", 5],
        ["2023-01-03", 4],
        ["2023-01-04", 4],
        ["2023-01-05", 5],
        ["2023-01-06", 4],
        ["2023-01-07", 3],
        ["2023-01-08", 3],
        ["2023-01-09", 4],
        ["2023-01-10", 3],
        ["2023-01-11", 3],
        ["2023-01-12", 3],
        ["2023-01-13", 4],
        ["2023-01-14", 5],
        ["2023-01-15", 5],
        ["2023-01-16", 3],
        ["2023-01-17", 3],
        ["2023-01-18", 3],
        ["2023-01-19", 4],
        ["2023-01-20", 3],
        ["2023-01-21", 3],
        ["2023-01-22", 5],
        ["2023-01-23", 4],
        ["2023-01-24", 3],
        ["2023-01-25", 3],
        ["2023-01-26", 3],
        ["2023-01-27", 3],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Февраль",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "3.83",
      dates: [
        ["2023-02-01", 3],
        ["2023-02-02", 4],
        ["2023-02-03", 4],
        ["2023-02-04", 3],
        ["2023-02-05", 5],
        ["2023-02-06", 3],
        ["2023-02-07", 4],
        ["2023-02-08", 4],
        ["2023-02-09", 4],
        ["2023-02-10", 3],
        ["2023-02-11", 4],
        ["2023-02-12", 5],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Март",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "3.97",
      dates: [
        ["2023-03-01", 4],
        ["2023-03-02", 5],
        ["2023-03-03", 3],
        ["2023-03-04", 4],
        ["2023-03-05", 3],
        ["2023-03-06", 3],
        ["2023-03-07", 3],
        ["2023-03-08", 3],
        ["2023-03-09", 3],
        ["2023-03-10", 3],
        ["2023-03-11", 3],
        ["2023-03-12", 5],
        ["2023-03-13", 4],
        ["2023-03-14", 5],
        ["2023-03-15", 3],
        ["2023-03-16", 5],
        ["2023-03-17", 5],
        ["2023-03-18", 5],
        ["2023-03-19", 3],
        ["2023-03-20", 3],
        ["2023-03-21", 3],
        ["2023-03-22", 4],
        ["2023-03-23", 5],
        ["2023-03-24", 5],
        ["2023-03-25", 5],
        ["2023-03-26", 4],
        ["2023-03-27", 5],
        ["2023-03-28", 5],
        ["2023-03-29", 4],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Апрель",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "3.96",
      dates: [
        ["2023-04-01", 3],
        ["2023-04-02", 3],
        ["2023-04-03", 5],
        ["2023-04-04", 5],
        ["2023-04-05", 3],
        ["2023-04-06", 5],
        ["2023-04-07", 5],
        ["2023-04-08", 3],
        ["2023-04-09", 4],
        ["2023-04-10", 3],
        ["2023-04-11", 3],
        ["2023-04-12", 5],
        ["2023-04-13", 4],
        ["2023-04-14", 4],
        ["2023-04-15", 5],
        ["2023-04-16", 5],
        ["2023-04-17", 4],
        ["2023-04-18", 4],
        ["2023-04-19", 3],
        ["2023-04-20", 3],
        ["2023-04-21", 4],
        ["2023-04-22", 4],
        ["2023-04-23", 4],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Май",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "4.35",
      dates: [
        ["2023-05-01", 4],
        ["2023-05-02", 5],
        ["2023-05-03", 5],
        ["2023-05-04", 3],
        ["2023-05-05", 5],
        ["2023-05-06", 4],
        ["2023-05-07", 4],
        ["2023-05-08", 4],
        ["2023-05-09", 4],
        ["2023-05-10", 5],
        ["2023-05-11", 4],
        ["2023-05-12", 3],
        ["2023-05-13", 5],
        ["2023-05-14", 4],
        ["2023-05-15", 5],
        ["2023-05-16", 5],
        ["2023-05-17", 4],
        ["2023-05-18", 5],
        ["2023-05-19", 5],
        ["2023-05-20", 4],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Июнь",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "3.85",
      dates: [
        ["2023-06-01", 4],
        ["2023-06-02", 4],
        ["2023-06-03", 4],
        ["2023-06-04", 5],
        ["2023-06-05", 4],
        ["2023-06-06", 4],
        ["2023-06-07", 4],
        ["2023-06-08", 5],
        ["2023-06-09", 5],
        ["2023-06-10", 3],
        ["2023-06-11", 3],
        ["2023-06-12", 3],
        ["2023-06-13", 3],
        ["2023-06-14", 3],
        ["2023-06-15", 5],
        ["2023-06-16", 3],
        ["2023-06-17", 5],
        ["2023-06-18", 4],
        ["2023-06-19", 3],
        ["2023-06-20", 3],
        ["2023-06-21", 4],
        ["2023-06-22", 5],
        ["2023-06-23", 5],
        ["2023-06-24", 3],
        ["2023-06-25", 3],
        ["2023-06-26", 3],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Сентябрь",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "3.93",
      dates: [
        ["2023-09-01", 5],
        ["2023-09-02", 5],
        ["2023-09-03", 3],
        ["2023-09-04", 4],
        ["2023-09-05", 5],
        ["2023-09-06", 5],
        ["2023-09-07", 5],
        ["2023-09-08", 4],
        ["2023-09-09", 3],
        ["2023-09-10", 3],
        ["2023-09-11", 4],
        ["2023-09-12", 4],
        ["2023-09-13", 4],
        ["2023-09-14", 3],
        ["2023-09-15", 5],
        ["2023-09-16", 4],
        ["2023-09-17", 4],
        ["2023-09-18", 3],
        ["2023-09-19", 5],
        ["2023-09-20", 4],
        ["2023-09-21", 4],
        ["2023-09-22", 4],
        ["2023-09-23", 3],
        ["2023-09-24", 3],
        ["2023-09-25", 4],
        ["2023-09-26", 4],
        ["2023-09-27", 3],
        ["2023-09-28", 3],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Октябрь",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "4.14",
      dates: [
        ["2023-10-01", 5],
        ["2023-10-02", 3],
        ["2023-10-03", 3],
        ["2023-10-04", 4],
        ["2023-10-05", 5],
        ["2023-10-06", 5],
        ["2023-10-07", 3],
        ["2023-10-08", 3],
        ["2023-10-09", 3],
        ["2023-10-10", 5],
        ["2023-10-11", 4],
        ["2023-10-12", 5],
        ["2023-10-13", 5],
        ["2023-10-14", 4],
        ["2023-10-15", 3],
        ["2023-10-16", 4],
        ["2023-10-17", 5],
        ["2023-10-18", 4],
        ["2023-10-19", 5],
        ["2023-10-20", 5],
        ["2023-10-21", 5],
        ["2023-10-22", 5],
        ["2023-10-23", 4],
        ["2023-10-24", 4],
        ["2023-10-25", 5],
        ["2023-10-26", 3],
        ["2023-10-27", 4],
        ["2023-10-28", 3],
        ["2023-10-29", 4],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Ноябрь",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "4.35",
      dates: [
        ["2023-11-01", 4],
        ["2023-11-02", 4],
        ["2023-11-03", 5],
        ["2023-11-04", 5],
        ["2023-11-05", 5],
        ["2023-11-06", 4],
        ["2023-11-07", 5],
        ["2023-11-08", 4],
        ["2023-11-09", 4],
        ["2023-11-10", 5],
        ["2023-11-11", 3],
        ["2023-11-12", 4],
        ["2023-11-13", 5],
        ["2023-11-14", 5],
        ["2023-11-15", 4],
        ["2023-11-16", 5],
        ["2023-11-17", 4],
        ["2023-11-18", 4],
        ["2023-11-19", 5],
        ["2023-11-20", 5],
        ["2023-11-21", 5],
        ["2023-11-22", 4],
        ["2023-11-23", 4],
        ["2023-11-24", 4],
        ["2023-11-25", 3],
        ["2023-11-26", 4],
      ],
    },
  },
  {
    subject: "Объектно-ориентированное программирование",
    month: "Декабрь",
    grades: {
      subject: "Объектно-ориентированное программирование",
      month_avg: "3.97",
      dates: [
        ["2023-12-01", 5],
        ["2023-12-02", 4],
        ["2023-12-03", 4],
        ["2023-12-04", 3],
        ["2023-12-05", 3],
        ["2023-12-06", 4],
        ["2023-12-07", 3],
        ["2023-12-08", 3],
        ["2023-12-09", 5],
        ["2023-12-10", 4],
        ["2023-12-11", 4],
        ["2023-12-12", 5],
        ["2023-12-13", 5],
        ["2023-12-14", 5],
        ["2023-12-15", 4],
        ["2023-12-16", 3],
        ["2023-12-17", 4],
        ["2023-12-18", 3],
        ["2023-12-19", 5],
        ["2023-12-20", 3],
        ["2023-12-21", 4],
        ["2023-12-22", 4],
        ["2023-12-23", 4],
        ["2023-12-24", 4],
        ["2023-12-25", 5],
        ["2023-12-26", 4],
        ["2023-12-27", 3],
        ["2023-12-28", 4],
        ["2023-12-29", 5],
        ["2023-12-30", 3],
        ["2023-12-31", 4],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Январь",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "3.89",
      dates: [
        ["2023-01-01", 4],
        ["2023-01-02", 3],
        ["2023-01-03", 4],
        ["2023-01-04", 5],
        ["2023-01-05", 4],
        ["2023-01-06", 4],
        ["2023-01-07", 3],
        ["2023-01-08", 4],
        ["2023-01-09", 4],
        ["2023-01-10", 5],
        ["2023-01-11", 3],
        ["2023-01-12", 4],
        ["2023-01-13", 3],
        ["2023-01-14", 5],
        ["2023-01-15", 3],
        ["2023-01-16", 3],
        ["2023-01-17", 5],
        ["2023-01-18", 4],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Февраль",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "3.86",
      dates: [
        ["2023-02-01", 3],
        ["2023-02-02", 4],
        ["2023-02-03", 5],
        ["2023-02-04", 3],
        ["2023-02-05", 4],
        ["2023-02-06", 4],
        ["2023-02-07", 3],
        ["2023-02-08", 3],
        ["2023-02-09", 5],
        ["2023-02-10", 5],
        ["2023-02-11", 4],
        ["2023-02-12", 4],
        ["2023-02-13", 4],
        ["2023-02-14", 3],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Март",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "3.88",
      dates: [
        ["2023-03-01", 4],
        ["2023-03-02", 3],
        ["2023-03-03", 4],
        ["2023-03-04", 4],
        ["2023-03-05", 4],
        ["2023-03-06", 3],
        ["2023-03-07", 5],
        ["2023-03-08", 4],
        ["2023-03-09", 5],
        ["2023-03-10", 5],
        ["2023-03-11", 4],
        ["2023-03-12", 3],
        ["2023-03-13", 4],
        ["2023-03-14", 4],
        ["2023-03-15", 3],
        ["2023-03-16", 4],
        ["2023-03-17", 5],
        ["2023-03-18", 5],
        ["2023-03-19", 3],
        ["2023-03-20", 3],
        ["2023-03-21", 3],
        ["2023-03-22", 4],
        ["2023-03-23", 4],
        ["2023-03-24", 3],
        ["2023-03-25", 4],
        ["2023-03-26", 4],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Апрель",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "3.81",
      dates: [
        ["2023-04-01", 4],
        ["2023-04-02", 4],
        ["2023-04-03", 4],
        ["2023-04-04", 3],
        ["2023-04-05", 3],
        ["2023-04-06", 4],
        ["2023-04-07", 3],
        ["2023-04-08", 3],
        ["2023-04-09", 5],
        ["2023-04-10", 4],
        ["2023-04-11", 4],
        ["2023-04-12", 3],
        ["2023-04-13", 3],
        ["2023-04-14", 5],
        ["2023-04-15", 4],
        ["2023-04-16", 5],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Май",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "4.27",
      dates: [
        ["2023-05-01", 4],
        ["2023-05-02", 3],
        ["2023-05-03", 5],
        ["2023-05-04", 4],
        ["2023-05-05", 4],
        ["2023-05-06", 5],
        ["2023-05-07", 4],
        ["2023-05-08", 5],
        ["2023-05-09", 5],
        ["2023-05-10", 5],
        ["2023-05-11", 3],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Июнь",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "3.90",
      dates: [
        ["2023-06-01", 3],
        ["2023-06-02", 4],
        ["2023-06-03", 5],
        ["2023-06-04", 4],
        ["2023-06-05", 3],
        ["2023-06-06", 3],
        ["2023-06-07", 5],
        ["2023-06-08", 3],
        ["2023-06-09", 4],
        ["2023-06-10", 3],
        ["2023-06-11", 5],
        ["2023-06-12", 5],
        ["2023-06-13", 3],
        ["2023-06-14", 3],
        ["2023-06-15", 4],
        ["2023-06-16", 3],
        ["2023-06-17", 5],
        ["2023-06-18", 4],
        ["2023-06-19", 5],
        ["2023-06-20", 4],
        ["2023-06-21", 4],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Сентябрь",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "4.00",
      dates: [
        ["2023-09-01", 3],
        ["2023-09-02", 4],
        ["2023-09-03", 3],
        ["2023-09-04", 5],
        ["2023-09-05", 4],
        ["2023-09-06", 5],
        ["2023-09-07", 3],
        ["2023-09-08", 3],
        ["2023-09-09", 5],
        ["2023-09-10", 3],
        ["2023-09-11", 5],
        ["2023-09-12", 3],
        ["2023-09-13", 5],
        ["2023-09-14", 4],
        ["2023-09-15", 5],
        ["2023-09-16", 5],
        ["2023-09-17", 3],
        ["2023-09-18", 4],
        ["2023-09-19", 4],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Октябрь",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "3.91",
      dates: [
        ["2023-10-01", 5],
        ["2023-10-02", 3],
        ["2023-10-03", 5],
        ["2023-10-04", 5],
        ["2023-10-05", 3],
        ["2023-10-06", 3],
        ["2023-10-07", 4],
        ["2023-10-08", 4],
        ["2023-10-09", 3],
        ["2023-10-10", 4],
        ["2023-10-11", 3],
        ["2023-10-12", 5],
        ["2023-10-13", 4],
        ["2023-10-14", 3],
        ["2023-10-15", 5],
        ["2023-10-16", 3],
        ["2023-10-17", 3],
        ["2023-10-18", 5],
        ["2023-10-19", 5],
        ["2023-10-20", 5],
        ["2023-10-21", 3],
        ["2023-10-22", 3],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Ноябрь",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "3.86",
      dates: [
        ["2023-11-01", 3],
        ["2023-11-02", 4],
        ["2023-11-03", 4],
        ["2023-11-04", 4],
        ["2023-11-05", 3],
        ["2023-11-06", 4],
        ["2023-11-07", 4],
        ["2023-11-08", 5],
        ["2023-11-09", 5],
        ["2023-11-10", 4],
        ["2023-11-11", 3],
        ["2023-11-12", 3],
        ["2023-11-13", 3],
        ["2023-11-14", 3],
        ["2023-11-15", 4],
        ["2023-11-16", 4],
        ["2023-11-17", 3],
        ["2023-11-18", 4],
        ["2023-11-19", 5],
        ["2023-11-20", 4],
        ["2023-11-21", 5],
      ],
    },
  },
  {
    subject: "Компьютерные сети",
    month: "Декабрь",
    grades: {
      subject: "Компьютерные сети",
      month_avg: "4.45",
      dates: [
        ["2023-12-01", 4],
        ["2023-12-02", 4],
        ["2023-12-03", 3],
        ["2023-12-04", 4],
        ["2023-12-05", 5],
        ["2023-12-06", 5],
        ["2023-12-07", 5],
        ["2023-12-08", 5],
        ["2023-12-09", 5],
        ["2023-12-10", 5],
        ["2023-12-11", 4],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Январь",
    grades: {
      subject: "Базы данных",
      month_avg: "3.93",
      dates: [
        ["2023-01-01", 4],
        ["2023-01-02", 4],
        ["2023-01-03", 3],
        ["2023-01-04", 3],
        ["2023-01-05", 3],
        ["2023-01-06", 3],
        ["2023-01-07", 3],
        ["2023-01-08", 5],
        ["2023-01-09", 3],
        ["2023-01-10", 5],
        ["2023-01-11", 3],
        ["2023-01-12", 5],
        ["2023-01-13", 4],
        ["2023-01-14", 5],
        ["2023-01-15", 5],
        ["2023-01-16", 3],
        ["2023-01-17", 3],
        ["2023-01-18", 3],
        ["2023-01-19", 5],
        ["2023-01-20", 5],
        ["2023-01-21", 3],
        ["2023-01-22", 5],
        ["2023-01-23", 5],
        ["2023-01-24", 3],
        ["2023-01-25", 4],
        ["2023-01-26", 5],
        ["2023-01-27", 3],
        ["2023-01-28", 4],
        ["2023-01-29", 5],
        ["2023-01-30", 4],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Февраль",
    grades: {
      subject: "Базы данных",
      month_avg: "3.75",
      dates: [
        ["2023-02-01", 5],
        ["2023-02-02", 5],
        ["2023-02-03", 3],
        ["2023-02-04", 3],
        ["2023-02-05", 4],
        ["2023-02-06", 3],
        ["2023-02-07", 3],
        ["2023-02-08", 3],
        ["2023-02-09", 3],
        ["2023-02-10", 4],
        ["2023-02-11", 4],
        ["2023-02-12", 5],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Март",
    grades: {
      subject: "Базы данных",
      month_avg: "4.00",
      dates: [
        ["2023-03-01", 4],
        ["2023-03-02", 5],
        ["2023-03-03", 5],
        ["2023-03-04", 5],
        ["2023-03-05", 3],
        ["2023-03-06", 5],
        ["2023-03-07", 3],
        ["2023-03-08", 3],
        ["2023-03-09", 3],
        ["2023-03-10", 5],
        ["2023-03-11", 3],
        ["2023-03-12", 4],
        ["2023-03-13", 4],
        ["2023-03-14", 4],
        ["2023-03-15", 3],
        ["2023-03-16", 4],
        ["2023-03-17", 4],
        ["2023-03-18", 5],
        ["2023-03-19", 3],
        ["2023-03-20", 4],
        ["2023-03-21", 4],
        ["2023-03-22", 4],
        ["2023-03-23", 4],
        ["2023-03-24", 4],
        ["2023-03-25", 5],
        ["2023-03-26", 3],
        ["2023-03-27", 5],
        ["2023-03-28", 5],
        ["2023-03-29", 3],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Апрель",
    grades: {
      subject: "Базы данных",
      month_avg: "3.73",
      dates: [
        ["2023-04-01", 4],
        ["2023-04-02", 3],
        ["2023-04-03", 3],
        ["2023-04-04", 4],
        ["2023-04-05", 5],
        ["2023-04-06", 3],
        ["2023-04-07", 5],
        ["2023-04-08", 3],
        ["2023-04-09", 4],
        ["2023-04-10", 3],
        ["2023-04-11", 4],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Май",
    grades: {
      subject: "Базы данных",
      month_avg: "4.09",
      dates: [
        ["2023-05-01", 5],
        ["2023-05-02", 5],
        ["2023-05-03", 3],
        ["2023-05-04", 3],
        ["2023-05-05", 5],
        ["2023-05-06", 4],
        ["2023-05-07", 4],
        ["2023-05-08", 3],
        ["2023-05-09", 5],
        ["2023-05-10", 4],
        ["2023-05-11", 4],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Июнь",
    grades: {
      subject: "Базы данных",
      month_avg: "4.04",
      dates: [
        ["2023-06-01", 5],
        ["2023-06-02", 3],
        ["2023-06-03", 4],
        ["2023-06-04", 3],
        ["2023-06-05", 4],
        ["2023-06-06", 4],
        ["2023-06-07", 3],
        ["2023-06-08", 4],
        ["2023-06-09", 5],
        ["2023-06-10", 5],
        ["2023-06-11", 5],
        ["2023-06-12", 4],
        ["2023-06-13", 5],
        ["2023-06-14", 5],
        ["2023-06-15", 4],
        ["2023-06-16", 3],
        ["2023-06-17", 5],
        ["2023-06-18", 4],
        ["2023-06-19", 3],
        ["2023-06-20", 3],
        ["2023-06-21", 4],
        ["2023-06-22", 4],
        ["2023-06-23", 4],
        ["2023-06-24", 4],
        ["2023-06-25", 4],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Сентябрь",
    grades: {
      subject: "Базы данных",
      month_avg: "4.00",
      dates: [
        ["2023-09-01", 4],
        ["2023-09-02", 3],
        ["2023-09-03", 4],
        ["2023-09-04", 3],
        ["2023-09-05", 3],
        ["2023-09-06", 5],
        ["2023-09-07", 3],
        ["2023-09-08", 4],
        ["2023-09-09", 5],
        ["2023-09-10", 4],
        ["2023-09-11", 5],
        ["2023-09-12", 3],
        ["2023-09-13", 5],
        ["2023-09-14", 4],
        ["2023-09-15", 5],
        ["2023-09-16", 5],
        ["2023-09-17", 3],
        ["2023-09-18", 4],
        ["2023-09-19", 4],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Октябрь",
    grades: {
      subject: "Базы данных",
      month_avg: "3.83",
      dates: [
        ["2023-10-01", 3],
        ["2023-10-02", 3],
        ["2023-10-03", 3],
        ["2023-10-04", 3],
        ["2023-10-05", 5],
        ["2023-10-06", 5],
        ["2023-10-07", 4],
        ["2023-10-08", 5],
        ["2023-10-09", 4],
        ["2023-10-10", 3],
        ["2023-10-11", 3],
        ["2023-10-12", 4],
        ["2023-10-13", 5],
        ["2023-10-14", 4],
        ["2023-10-15", 4],
        ["2023-10-16", 3],
        ["2023-10-17", 3],
        ["2023-10-18", 4],
        ["2023-10-19", 5],
        ["2023-10-20", 4],
        ["2023-10-21", 3],
        ["2023-10-22", 5],
        ["2023-10-23", 4],
        ["2023-10-24", 3],
        ["2023-10-25", 4],
        ["2023-10-26", 5],
        ["2023-10-27", 3],
        ["2023-10-28", 4],
        ["2023-10-29", 3],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Ноябрь",
    grades: {
      subject: "Базы данных",
      month_avg: "4.00",
      dates: [
        ["2023-11-01", 5],
        ["2023-11-02", 4],
        ["2023-11-03", 4],
        ["2023-11-04", 3],
        ["2023-11-05", 3],
        ["2023-11-06", 4],
        ["2023-11-07", 4],
        ["2023-11-08", 3],
        ["2023-11-09", 3],
        ["2023-11-10", 4],
        ["2023-11-11", 5],
        ["2023-11-12", 3],
        ["2023-11-13", 5],
        ["2023-11-14", 4],
        ["2023-11-15", 5],
        ["2023-11-16", 5],
      ],
    },
  },
  {
    subject: "Базы данных",
    month: "Декабрь",
    grades: {
      subject: "Базы данных",
      month_avg: "4.10",
      dates: [
        ["2023-12-01", 5],
        ["2023-12-02", 5],
        ["2023-12-03", 3],
        ["2023-12-04", 4],
        ["2023-12-05", 4],
        ["2023-12-06", 4],
        ["2023-12-07", 4],
        ["2023-12-08", 3],
        ["2023-12-09", 5],
        ["2023-12-10", 4],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Январь",
    grades: {
      subject: "Математика",
      month_avg: "3.93",
      dates: [
        ["2023-01-01", 4],
        ["2023-01-02", 4],
        ["2023-01-03", 4],
        ["2023-01-04", 5],
        ["2023-01-05", 4],
        ["2023-01-06", 5],
        ["2023-01-07", 3],
        ["2023-01-08", 5],
        ["2023-01-09", 3],
        ["2023-01-10", 4],
        ["2023-01-11", 3],
        ["2023-01-12", 3],
        ["2023-01-13", 3],
        ["2023-01-14", 4],
        ["2023-01-15", 5],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Февраль",
    grades: {
      subject: "Математика",
      month_avg: "3.68",
      dates: [
        ["2023-02-01", 3],
        ["2023-02-02", 4],
        ["2023-02-03", 3],
        ["2023-02-04", 3],
        ["2023-02-05", 5],
        ["2023-02-06", 4],
        ["2023-02-07", 3],
        ["2023-02-08", 3],
        ["2023-02-09", 3],
        ["2023-02-10", 3],
        ["2023-02-11", 5],
        ["2023-02-12", 3],
        ["2023-02-13", 3],
        ["2023-02-14", 5],
        ["2023-02-15", 3],
        ["2023-02-16", 4],
        ["2023-02-17", 3],
        ["2023-02-18", 5],
        ["2023-02-19", 3],
        ["2023-02-20", 4],
        ["2023-02-21", 4],
        ["2023-02-22", 3],
        ["2023-02-23", 4],
        ["2023-02-24", 5],
        ["2023-02-25", 3],
        ["2023-02-26", 5],
        ["2023-02-27", 3],
        ["2023-02-28", 4],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Март",
    grades: {
      subject: "Математика",
      month_avg: "3.92",
      dates: [
        ["2023-03-01", 3],
        ["2023-03-02", 3],
        ["2023-03-03", 5],
        ["2023-03-04", 5],
        ["2023-03-05", 3],
        ["2023-03-06", 3],
        ["2023-03-07", 3],
        ["2023-03-08", 5],
        ["2023-03-09", 4],
        ["2023-03-10", 5],
        ["2023-03-11", 4],
        ["2023-03-12", 4],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Апрель",
    grades: {
      subject: "Математика",
      month_avg: "4.42",
      dates: [
        ["2023-04-01", 5],
        ["2023-04-02", 5],
        ["2023-04-03", 5],
        ["2023-04-04", 4],
        ["2023-04-05", 5],
        ["2023-04-06", 4],
        ["2023-04-07", 5],
        ["2023-04-08", 5],
        ["2023-04-09", 5],
        ["2023-04-10", 5],
        ["2023-04-11", 3],
        ["2023-04-12", 5],
        ["2023-04-13", 5],
        ["2023-04-14", 4],
        ["2023-04-15", 3],
        ["2023-04-16", 5],
        ["2023-04-17", 4],
        ["2023-04-18", 5],
        ["2023-04-19", 4],
        ["2023-04-20", 4],
        ["2023-04-21", 3],
        ["2023-04-22", 4],
        ["2023-04-23", 4],
        ["2023-04-24", 5],
        ["2023-04-25", 5],
        ["2023-04-26", 4],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Май",
    grades: {
      subject: "Математика",
      month_avg: "3.83",
      dates: [
        ["2023-05-01", 3],
        ["2023-05-02", 5],
        ["2023-05-03", 4],
        ["2023-05-04", 4],
        ["2023-05-05", 4],
        ["2023-05-06", 4],
        ["2023-05-07", 4],
        ["2023-05-08", 3],
        ["2023-05-09", 5],
        ["2023-05-10", 3],
        ["2023-05-11", 3],
        ["2023-05-12", 4],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Июнь",
    grades: {
      subject: "Математика",
      month_avg: "3.88",
      dates: [
        ["2023-06-01", 5],
        ["2023-06-02", 4],
        ["2023-06-03", 4],
        ["2023-06-04", 4],
        ["2023-06-05", 5],
        ["2023-06-06", 3],
        ["2023-06-07", 5],
        ["2023-06-08", 4],
        ["2023-06-09", 5],
        ["2023-06-10", 4],
        ["2023-06-11", 3],
        ["2023-06-12", 3],
        ["2023-06-13", 3],
        ["2023-06-14", 3],
        ["2023-06-15", 5],
        ["2023-06-16", 3],
        ["2023-06-17", 3],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Сентябрь",
    grades: {
      subject: "Математика",
      month_avg: "3.75",
      dates: [
        ["2023-09-01", 3],
        ["2023-09-02", 5],
        ["2023-09-03", 3],
        ["2023-09-04", 3],
        ["2023-09-05", 5],
        ["2023-09-06", 5],
        ["2023-09-07", 4],
        ["2023-09-08", 3],
        ["2023-09-09", 4],
        ["2023-09-10", 5],
        ["2023-09-11", 3],
        ["2023-09-12", 5],
        ["2023-09-13", 4],
        ["2023-09-14", 5],
        ["2023-09-15", 3],
        ["2023-09-16", 3],
        ["2023-09-17", 3],
        ["2023-09-18", 4],
        ["2023-09-19", 3],
        ["2023-09-20", 3],
        ["2023-09-21", 3],
        ["2023-09-22", 4],
        ["2023-09-23", 5],
        ["2023-09-24", 3],
        ["2023-09-25", 3],
        ["2023-09-26", 3],
        ["2023-09-27", 5],
        ["2023-09-28", 3],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Октябрь",
    grades: {
      subject: "Математика",
      month_avg: "3.90",
      dates: [
        ["2023-10-01", 3],
        ["2023-10-02", 3],
        ["2023-10-03", 5],
        ["2023-10-04", 4],
        ["2023-10-05", 4],
        ["2023-10-06", 5],
        ["2023-10-07", 4],
        ["2023-10-08", 4],
        ["2023-10-09", 4],
        ["2023-10-10", 3],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Ноябрь",
    grades: {
      subject: "Математика",
      month_avg: "3.90",
      dates: [
        ["2023-11-01", 4],
        ["2023-11-02", 4],
        ["2023-11-03", 3],
        ["2023-11-04", 4],
        ["2023-11-05", 3],
        ["2023-11-06", 5],
        ["2023-11-07", 4],
        ["2023-11-08", 5],
        ["2023-11-09", 4],
        ["2023-11-10", 3],
        ["2023-11-11", 5],
        ["2023-11-12", 3],
        ["2023-11-13", 5],
        ["2023-11-14", 4],
        ["2023-11-15", 5],
        ["2023-11-16", 5],
        ["2023-11-17", 3],
        ["2023-11-18", 3],
        ["2023-11-19", 4],
        ["2023-11-20", 3],
        ["2023-11-21", 3],
        ["2023-11-22", 4],
        ["2023-11-23", 4],
        ["2023-11-24", 4],
        ["2023-11-25", 4],
        ["2023-11-26", 3],
        ["2023-11-27", 4],
        ["2023-11-28", 3],
        ["2023-11-29", 4],
        ["2023-11-30", 5],
      ],
    },
  },
  {
    subject: "Математика",
    month: "Декабрь",
    grades: {
      subject: "Математика",
      month_avg: "4.00",
      dates: [
        ["2023-12-01", 4],
        ["2023-12-02", 4],
        ["2023-12-03", 4],
        ["2023-12-04", 3],
        ["2023-12-05", 5],
        ["2023-12-06", 3],
        ["2023-12-07", 4],
        ["2023-12-08", 4],
        ["2023-12-09", 5],
        ["2023-12-10", 5],
        ["2023-12-11", 4],
        ["2023-12-12", 3],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Январь",
    grades: {
      subject: "Английский язык",
      month_avg: "4.38",
      dates: [
        ["2023-01-01", 4],
        ["2023-01-02", 3],
        ["2023-01-03", 3],
        ["2023-01-04", 5],
        ["2023-01-05", 5],
        ["2023-01-06", 5],
        ["2023-01-07", 5],
        ["2023-01-08", 5],
        ["2023-01-09", 4],
        ["2023-01-10", 4],
        ["2023-01-11", 5],
        ["2023-01-12", 5],
        ["2023-01-13", 4],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Февраль",
    grades: {
      subject: "Английский язык",
      month_avg: "4.09",
      dates: [
        ["2023-02-01", 3],
        ["2023-02-02", 4],
        ["2023-02-03", 5],
        ["2023-02-04", 3],
        ["2023-02-05", 3],
        ["2023-02-06", 4],
        ["2023-02-07", 5],
        ["2023-02-08", 5],
        ["2023-02-09", 4],
        ["2023-02-10", 5],
        ["2023-02-11", 5],
        ["2023-02-12", 4],
        ["2023-02-13", 3],
        ["2023-02-14", 5],
        ["2023-02-15", 3],
        ["2023-02-16", 5],
        ["2023-02-17", 5],
        ["2023-02-18", 3],
        ["2023-02-19", 4],
        ["2023-02-20", 5],
        ["2023-02-21", 3],
        ["2023-02-22", 4],
        ["2023-02-23", 4],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Март",
    grades: {
      subject: "Английский язык",
      month_avg: "3.88",
      dates: [
        ["2023-03-01", 4],
        ["2023-03-02", 5],
        ["2023-03-03", 5],
        ["2023-03-04", 4],
        ["2023-03-05", 4],
        ["2023-03-06", 4],
        ["2023-03-07", 3],
        ["2023-03-08", 3],
        ["2023-03-09", 4],
        ["2023-03-10", 3],
        ["2023-03-11", 3],
        ["2023-03-12", 4],
        ["2023-03-13", 4],
        ["2023-03-14", 5],
        ["2023-03-15", 4],
        ["2023-03-16", 5],
        ["2023-03-17", 4],
        ["2023-03-18", 4],
        ["2023-03-19", 3],
        ["2023-03-20", 3],
        ["2023-03-21", 4],
        ["2023-03-22", 3],
        ["2023-03-23", 5],
        ["2023-03-24", 3],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Апрель",
    grades: {
      subject: "Английский язык",
      month_avg: "4.00",
      dates: [
        ["2023-04-01", 4],
        ["2023-04-02", 5],
        ["2023-04-03", 4],
        ["2023-04-04", 3],
        ["2023-04-05", 3],
        ["2023-04-06", 4],
        ["2023-04-07", 5],
        ["2023-04-08", 4],
        ["2023-04-09", 4],
        ["2023-04-10", 3],
        ["2023-04-11", 3],
        ["2023-04-12", 4],
        ["2023-04-13", 4],
        ["2023-04-14", 5],
        ["2023-04-15", 5],
        ["2023-04-16", 3],
        ["2023-04-17", 5],
        ["2023-04-18", 5],
        ["2023-04-19", 3],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Май",
    grades: {
      subject: "Английский язык",
      month_avg: "3.96",
      dates: [
        ["2023-05-01", 4],
        ["2023-05-02", 3],
        ["2023-05-03", 3],
        ["2023-05-04", 3],
        ["2023-05-05", 4],
        ["2023-05-06", 5],
        ["2023-05-07", 4],
        ["2023-05-08", 4],
        ["2023-05-09", 5],
        ["2023-05-10", 5],
        ["2023-05-11", 5],
        ["2023-05-12", 3],
        ["2023-05-13", 3],
        ["2023-05-14", 3],
        ["2023-05-15", 4],
        ["2023-05-16", 5],
        ["2023-05-17", 4],
        ["2023-05-18", 4],
        ["2023-05-19", 4],
        ["2023-05-20", 3],
        ["2023-05-21", 5],
        ["2023-05-22", 3],
        ["2023-05-23", 3],
        ["2023-05-24", 3],
        ["2023-05-25", 5],
        ["2023-05-26", 4],
        ["2023-05-27", 5],
        ["2023-05-28", 5],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Июнь",
    grades: {
      subject: "Английский язык",
      month_avg: "4.00",
      dates: [
        ["2023-06-01", 4],
        ["2023-06-02", 4],
        ["2023-06-03", 4],
        ["2023-06-04", 5],
        ["2023-06-05", 4],
        ["2023-06-06", 5],
        ["2023-06-07", 4],
        ["2023-06-08", 4],
        ["2023-06-09", 4],
        ["2023-06-10", 5],
        ["2023-06-11", 3],
        ["2023-06-12", 4],
        ["2023-06-13", 3],
        ["2023-06-14", 3],
        ["2023-06-15", 4],
        ["2023-06-16", 4],
        ["2023-06-17", 5],
        ["2023-06-18", 4],
        ["2023-06-19", 5],
        ["2023-06-20", 5],
        ["2023-06-21", 3],
        ["2023-06-22", 3],
        ["2023-06-23", 4],
        ["2023-06-24", 3],
        ["2023-06-25", 4],
        ["2023-06-26", 4],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Сентябрь",
    grades: {
      subject: "Английский язык",
      month_avg: "4.14",
      dates: [
        ["2023-09-01", 4],
        ["2023-09-02", 4],
        ["2023-09-03", 4],
        ["2023-09-04", 4],
        ["2023-09-05", 3],
        ["2023-09-06", 4],
        ["2023-09-07", 4],
        ["2023-09-08", 4],
        ["2023-09-09", 4],
        ["2023-09-10", 5],
        ["2023-09-11", 3],
        ["2023-09-12", 5],
        ["2023-09-13", 5],
        ["2023-09-14", 3],
        ["2023-09-15", 5],
        ["2023-09-16", 5],
        ["2023-09-17", 5],
        ["2023-09-18", 3],
        ["2023-09-19", 3],
        ["2023-09-20", 5],
        ["2023-09-21", 5],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Октябрь",
    grades: {
      subject: "Английский язык",
      month_avg: "3.89",
      dates: [
        ["2023-10-01", 3],
        ["2023-10-02", 5],
        ["2023-10-03", 3],
        ["2023-10-04", 4],
        ["2023-10-05", 3],
        ["2023-10-06", 3],
        ["2023-10-07", 3],
        ["2023-10-08", 5],
        ["2023-10-09", 4],
        ["2023-10-10", 5],
        ["2023-10-11", 4],
        ["2023-10-12", 3],
        ["2023-10-13", 4],
        ["2023-10-14", 3],
        ["2023-10-15", 5],
        ["2023-10-16", 5],
        ["2023-10-17", 3],
        ["2023-10-18", 5],
        ["2023-10-19", 4],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Ноябрь",
    grades: {
      subject: "Английский язык",
      month_avg: "4.25",
      dates: [
        ["2023-11-01", 4],
        ["2023-11-02", 4],
        ["2023-11-03", 4],
        ["2023-11-04", 3],
        ["2023-11-05", 4],
        ["2023-11-06", 5],
        ["2023-11-07", 5],
        ["2023-11-08", 3],
        ["2023-11-09", 5],
        ["2023-11-10", 4],
        ["2023-11-11", 5],
        ["2023-11-12", 4],
        ["2023-11-13", 5],
        ["2023-11-14", 5],
        ["2023-11-15", 5],
        ["2023-11-16", 3],
      ],
    },
  },
  {
    subject: "Английский язык",
    month: "Декабрь",
    grades: {
      subject: "Английский язык",
      month_avg: "4.00",
      dates: [
        ["2023-12-01", 4],
        ["2023-12-02", 5],
        ["2023-12-03", 5],
        ["2023-12-04", 3],
        ["2023-12-05", 3],
        ["2023-12-06", 4],
        ["2023-12-07", 4],
        ["2023-12-08", 4],
        ["2023-12-09", 4],
        ["2023-12-10", 4],
        ["2023-12-11", 3],
        ["2023-12-12", 3],
        ["2023-12-13", 3],
        ["2023-12-14", 4],
        ["2023-12-15", 5],
        ["2023-12-16", 5],
        ["2023-12-17", 5],
        ["2023-12-18", 4],
        ["2023-12-19", 5],
        ["2023-12-20", 4],
        ["2023-12-21", 5],
        ["2023-12-22", 4],
        ["2023-12-23", 3],
        ["2023-12-24", 3],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Январь",
    grades: {
      subject: "Операционные системы",
      month_avg: "3.96",
      dates: [
        ["2023-01-01", 4],
        ["2023-01-02", 4],
        ["2023-01-03", 5],
        ["2023-01-04", 5],
        ["2023-01-05", 3],
        ["2023-01-06", 5],
        ["2023-01-07", 3],
        ["2023-01-08", 4],
        ["2023-01-09", 3],
        ["2023-01-10", 3],
        ["2023-01-11", 5],
        ["2023-01-12", 3],
        ["2023-01-13", 3],
        ["2023-01-14", 5],
        ["2023-01-15", 5],
        ["2023-01-16", 4],
        ["2023-01-17", 3],
        ["2023-01-18", 3],
        ["2023-01-19", 4],
        ["2023-01-20", 4],
        ["2023-01-21", 5],
        ["2023-01-22", 4],
        ["2023-01-23", 4],
        ["2023-01-24", 5],
        ["2023-01-25", 4],
        ["2023-01-26", 3],
        ["2023-01-27", 3],
        ["2023-01-28", 5],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Февраль",
    grades: {
      subject: "Операционные системы",
      month_avg: "4.10",
      dates: [
        ["2023-02-01", 5],
        ["2023-02-02", 5],
        ["2023-02-03", 3],
        ["2023-02-04", 4],
        ["2023-02-05", 5],
        ["2023-02-06", 3],
        ["2023-02-07", 5],
        ["2023-02-08", 5],
        ["2023-02-09", 3],
        ["2023-02-10", 4],
        ["2023-02-11", 4],
        ["2023-02-12", 4],
        ["2023-02-13", 5],
        ["2023-02-14", 5],
        ["2023-02-15", 5],
        ["2023-02-16", 5],
        ["2023-02-17", 3],
        ["2023-02-18", 3],
        ["2023-02-19", 3],
        ["2023-02-20", 4],
        ["2023-02-21", 3],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Март",
    grades: {
      subject: "Операционные системы",
      month_avg: "4.13",
      dates: [
        ["2023-03-01", 5],
        ["2023-03-02", 4],
        ["2023-03-03", 4],
        ["2023-03-04", 3],
        ["2023-03-05", 3],
        ["2023-03-06", 5],
        ["2023-03-07", 4],
        ["2023-03-08", 4],
        ["2023-03-09", 4],
        ["2023-03-10", 3],
        ["2023-03-11", 5],
        ["2023-03-12", 5],
        ["2023-03-13", 4],
        ["2023-03-14", 5],
        ["2023-03-15", 4],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Апрель",
    grades: {
      subject: "Операционные системы",
      month_avg: "4.08",
      dates: [
        ["2023-04-01", 3],
        ["2023-04-02", 5],
        ["2023-04-03", 4],
        ["2023-04-04", 5],
        ["2023-04-05", 3],
        ["2023-04-06", 4],
        ["2023-04-07", 3],
        ["2023-04-08", 4],
        ["2023-04-09", 3],
        ["2023-04-10", 5],
        ["2023-04-11", 5],
        ["2023-04-12", 4],
        ["2023-04-13", 5],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Май",
    grades: {
      subject: "Операционные системы",
      month_avg: "3.54",
      dates: [
        ["2023-05-01", 3],
        ["2023-05-02", 3],
        ["2023-05-03", 3],
        ["2023-05-04", 4],
        ["2023-05-05", 5],
        ["2023-05-06", 4],
        ["2023-05-07", 3],
        ["2023-05-08", 4],
        ["2023-05-09", 3],
        ["2023-05-10", 3],
        ["2023-05-11", 3],
        ["2023-05-12", 3],
        ["2023-05-13", 5],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Июнь",
    grades: {
      subject: "Операционные системы",
      month_avg: "3.92",
      dates: [
        ["2023-06-01", 4],
        ["2023-06-02", 3],
        ["2023-06-03", 5],
        ["2023-06-04", 4],
        ["2023-06-05", 3],
        ["2023-06-06", 3],
        ["2023-06-07", 5],
        ["2023-06-08", 5],
        ["2023-06-09", 3],
        ["2023-06-10", 4],
        ["2023-06-11", 5],
        ["2023-06-12", 3],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Сентябрь",
    grades: {
      subject: "Операционные системы",
      month_avg: "4.04",
      dates: [
        ["2023-09-01", 4],
        ["2023-09-02", 4],
        ["2023-09-03", 3],
        ["2023-09-04", 5],
        ["2023-09-05", 5],
        ["2023-09-06", 5],
        ["2023-09-07", 5],
        ["2023-09-08", 3],
        ["2023-09-09", 3],
        ["2023-09-10", 4],
        ["2023-09-11", 5],
        ["2023-09-12", 4],
        ["2023-09-13", 3],
        ["2023-09-14", 4],
        ["2023-09-15", 5],
        ["2023-09-16", 4],
        ["2023-09-17", 5],
        ["2023-09-18", 4],
        ["2023-09-19", 3],
        ["2023-09-20", 5],
        ["2023-09-21", 5],
        ["2023-09-22", 5],
        ["2023-09-23", 4],
        ["2023-09-24", 3],
        ["2023-09-25", 3],
        ["2023-09-26", 3],
        ["2023-09-27", 3],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Октябрь",
    grades: {
      subject: "Операционные системы",
      month_avg: "3.81",
      dates: [
        ["2023-10-01", 4],
        ["2023-10-02", 5],
        ["2023-10-03", 5],
        ["2023-10-04", 3],
        ["2023-10-05", 5],
        ["2023-10-06", 3],
        ["2023-10-07", 4],
        ["2023-10-08", 3],
        ["2023-10-09", 4],
        ["2023-10-10", 3],
        ["2023-10-11", 4],
        ["2023-10-12", 3],
        ["2023-10-13", 3],
        ["2023-10-14", 3],
        ["2023-10-15", 4],
        ["2023-10-16", 5],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Ноябрь",
    grades: {
      subject: "Операционные системы",
      month_avg: "4.21",
      dates: [
        ["2023-11-01", 3],
        ["2023-11-02", 5],
        ["2023-11-03", 5],
        ["2023-11-04", 4],
        ["2023-11-05", 3],
        ["2023-11-06", 5],
        ["2023-11-07", 3],
        ["2023-11-08", 5],
        ["2023-11-09", 4],
        ["2023-11-10", 5],
        ["2023-11-11", 4],
        ["2023-11-12", 3],
        ["2023-11-13", 4],
        ["2023-11-14", 5],
        ["2023-11-15", 3],
        ["2023-11-16", 5],
        ["2023-11-17", 5],
        ["2023-11-18", 3],
        ["2023-11-19", 4],
        ["2023-11-20", 5],
        ["2023-11-21", 5],
        ["2023-11-22", 4],
        ["2023-11-23", 4],
        ["2023-11-24", 5],
      ],
    },
  },
  {
    subject: "Операционные системы",
    month: "Декабрь",
    grades: {
      subject: "Операционные системы",
      month_avg: "3.48",
      dates: [
        ["2023-12-01", 5],
        ["2023-12-02", 5],
        ["2023-12-03", 3],
        ["2023-12-04", 3],
        ["2023-12-05", 4],
        ["2023-12-06", 3],
        ["2023-12-07", 3],
        ["2023-12-08", 3],
        ["2023-12-09", 4],
        ["2023-12-10", 3],
        ["2023-12-11", 4],
        ["2023-12-12", 3],
        ["2023-12-13", 4],
        ["2023-12-14", 4],
        ["2023-12-15", 3],
        ["2023-12-16", 3],
        ["2023-12-17", 3],
        ["2023-12-18", 4],
        ["2023-12-19", 4],
        ["2023-12-20", 3],
        ["2023-12-21", 3],
        ["2023-12-22", 3],
        ["2023-12-23", 3],
        ["2023-12-24", 3],
        ["2023-12-25", 4],
        ["2023-12-26", 4],
        ["2023-12-27", 3],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Январь",
    grades: {
      subject: "Веб-программирование",
      month_avg: "4.13",
      dates: [
        ["2023-01-01", 4],
        ["2023-01-02", 3],
        ["2023-01-03", 3],
        ["2023-01-04", 3],
        ["2023-01-05", 5],
        ["2023-01-06", 4],
        ["2023-01-07", 3],
        ["2023-01-08", 3],
        ["2023-01-09", 5],
        ["2023-01-10", 5],
        ["2023-01-11", 5],
        ["2023-01-12", 5],
        ["2023-01-13", 5],
        ["2023-01-14", 3],
        ["2023-01-15", 4],
        ["2023-01-16", 5],
        ["2023-01-17", 5],
        ["2023-01-18", 5],
        ["2023-01-19", 4],
        ["2023-01-20", 3],
        ["2023-01-21", 4],
        ["2023-01-22", 4],
        ["2023-01-23", 5],
        ["2023-01-24", 5],
        ["2023-01-25", 3],
        ["2023-01-26", 5],
        ["2023-01-27", 3],
        ["2023-01-28", 5],
        ["2023-01-29", 5],
        ["2023-01-30", 3],
        ["2023-01-31", 4],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Февраль",
    grades: {
      subject: "Веб-программирование",
      month_avg: "4.12",
      dates: [
        ["2023-02-01", 4],
        ["2023-02-02", 4],
        ["2023-02-03", 5],
        ["2023-02-04", 5],
        ["2023-02-05", 5],
        ["2023-02-06", 5],
        ["2023-02-07", 5],
        ["2023-02-08", 3],
        ["2023-02-09", 3],
        ["2023-02-10", 3],
        ["2023-02-11", 5],
        ["2023-02-12", 3],
        ["2023-02-13", 5],
        ["2023-02-14", 5],
        ["2023-02-15", 5],
        ["2023-02-16", 5],
        ["2023-02-17", 5],
        ["2023-02-18", 4],
        ["2023-02-19", 3],
        ["2023-02-20", 3],
        ["2023-02-21", 3],
        ["2023-02-22", 3],
        ["2023-02-23", 4],
        ["2023-02-24", 4],
        ["2023-02-25", 4],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Март",
    grades: {
      subject: "Веб-программирование",
      month_avg: "4.10",
      dates: [
        ["2023-03-01", 4],
        ["2023-03-02", 3],
        ["2023-03-03", 5],
        ["2023-03-04", 3],
        ["2023-03-05", 4],
        ["2023-03-06", 4],
        ["2023-03-07", 4],
        ["2023-03-08", 5],
        ["2023-03-09", 4],
        ["2023-03-10", 3],
        ["2023-03-11", 4],
        ["2023-03-12", 3],
        ["2023-03-13", 4],
        ["2023-03-14", 4],
        ["2023-03-15", 5],
        ["2023-03-16", 4],
        ["2023-03-17", 3],
        ["2023-03-18", 4],
        ["2023-03-19", 5],
        ["2023-03-20", 5],
        ["2023-03-21", 4],
        ["2023-03-22", 4],
        ["2023-03-23", 4],
        ["2023-03-24", 5],
        ["2023-03-25", 5],
        ["2023-03-26", 5],
        ["2023-03-27", 3],
        ["2023-03-28", 4],
        ["2023-03-29", 5],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Апрель",
    grades: {
      subject: "Веб-программирование",
      month_avg: "3.88",
      dates: [
        ["2023-04-01", 3],
        ["2023-04-02", 5],
        ["2023-04-03", 5],
        ["2023-04-04", 5],
        ["2023-04-05", 4],
        ["2023-04-06", 3],
        ["2023-04-07", 4],
        ["2023-04-08", 5],
        ["2023-04-09", 5],
        ["2023-04-10", 3],
        ["2023-04-11", 4],
        ["2023-04-12", 3],
        ["2023-04-13", 5],
        ["2023-04-14", 4],
        ["2023-04-15", 4],
        ["2023-04-16", 3],
        ["2023-04-17", 3],
        ["2023-04-18", 3],
        ["2023-04-19", 5],
        ["2023-04-20", 3],
        ["2023-04-21", 3],
        ["2023-04-22", 4],
        ["2023-04-23", 3],
        ["2023-04-24", 4],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Май",
    grades: {
      subject: "Веб-программирование",
      month_avg: "3.96",
      dates: [
        ["2023-05-01", 3],
        ["2023-05-02", 5],
        ["2023-05-03", 4],
        ["2023-05-04", 4],
        ["2023-05-05", 4],
        ["2023-05-06", 5],
        ["2023-05-07", 4],
        ["2023-05-08", 4],
        ["2023-05-09", 4],
        ["2023-05-10", 4],
        ["2023-05-11", 5],
        ["2023-05-12", 4],
        ["2023-05-13", 3],
        ["2023-05-14", 3],
        ["2023-05-15", 3],
        ["2023-05-16", 3],
        ["2023-05-17", 5],
        ["2023-05-18", 3],
        ["2023-05-19", 5],
        ["2023-05-20", 3],
        ["2023-05-21", 5],
        ["2023-05-22", 4],
        ["2023-05-23", 4],
        ["2023-05-24", 5],
        ["2023-05-25", 3],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Июнь",
    grades: {
      subject: "Веб-программирование",
      month_avg: "3.92",
      dates: [
        ["2023-06-01", 3],
        ["2023-06-02", 3],
        ["2023-06-03", 4],
        ["2023-06-04", 5],
        ["2023-06-05", 3],
        ["2023-06-06", 5],
        ["2023-06-07", 5],
        ["2023-06-08", 3],
        ["2023-06-09", 4],
        ["2023-06-10", 3],
        ["2023-06-11", 3],
        ["2023-06-12", 4],
        ["2023-06-13", 4],
        ["2023-06-14", 3],
        ["2023-06-15", 5],
        ["2023-06-16", 5],
        ["2023-06-17", 3],
        ["2023-06-18", 4],
        ["2023-06-19", 5],
        ["2023-06-20", 5],
        ["2023-06-21", 3],
        ["2023-06-22", 4],
        ["2023-06-23", 3],
        ["2023-06-24", 4],
        ["2023-06-25", 5],
        ["2023-06-26", 4],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Сентябрь",
    grades: {
      subject: "Веб-программирование",
      month_avg: "3.69",
      dates: [
        ["2023-09-01", 3],
        ["2023-09-02", 3],
        ["2023-09-03", 3],
        ["2023-09-04", 3],
        ["2023-09-05", 5],
        ["2023-09-06", 3],
        ["2023-09-07", 5],
        ["2023-09-08", 4],
        ["2023-09-09", 4],
        ["2023-09-10", 3],
        ["2023-09-11", 4],
        ["2023-09-12", 3],
        ["2023-09-13", 4],
        ["2023-09-14", 4],
        ["2023-09-15", 4],
        ["2023-09-16", 4],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Октябрь",
    grades: {
      subject: "Веб-программирование",
      month_avg: "4.08",
      dates: [
        ["2023-10-01", 4],
        ["2023-10-02", 4],
        ["2023-10-03", 4],
        ["2023-10-04", 4],
        ["2023-10-05", 4],
        ["2023-10-06", 3],
        ["2023-10-07", 5],
        ["2023-10-08", 4],
        ["2023-10-09", 5],
        ["2023-10-10", 4],
        ["2023-10-11", 3],
        ["2023-10-12", 5],
        ["2023-10-13", 4],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Ноябрь",
    grades: {
      subject: "Веб-программирование",
      month_avg: "4.00",
      dates: [
        ["2023-11-01", 5],
        ["2023-11-02", 4],
        ["2023-11-03", 5],
        ["2023-11-04", 5],
        ["2023-11-05", 5],
        ["2023-11-06", 3],
        ["2023-11-07", 3],
        ["2023-11-08", 3],
        ["2023-11-09", 5],
        ["2023-11-10", 5],
        ["2023-11-11", 3],
        ["2023-11-12", 3],
        ["2023-11-13", 3],
        ["2023-11-14", 3],
        ["2023-11-15", 5],
        ["2023-11-16", 4],
      ],
    },
  },
  {
    subject: "Веб-программирование",
    month: "Декабрь",
    grades: {
      subject: "Веб-программирование",
      month_avg: "3.93",
      dates: [
        ["2023-12-01", 4],
        ["2023-12-02", 3],
        ["2023-12-03", 3],
        ["2023-12-04", 3],
        ["2023-12-05", 3],
        ["2023-12-06", 3],
        ["2023-12-07", 5],
        ["2023-12-08", 3],
        ["2023-12-09", 4],
        ["2023-12-10", 4],
        ["2023-12-11", 5],
        ["2023-12-12", 4],
        ["2023-12-13", 5],
        ["2023-12-14", 4],
        ["2023-12-15", 3],
        ["2023-12-16", 3],
        ["2023-12-17", 3],
        ["2023-12-18", 4],
        ["2023-12-19", 5],
        ["2023-12-20", 3],
        ["2023-12-21", 3],
        ["2023-12-22", 5],
        ["2023-12-23", 4],
        ["2023-12-24", 4],
        ["2023-12-25", 5],
        ["2023-12-26", 5],
        ["2023-12-27", 4],
        ["2023-12-28", 4],
        ["2023-12-29", 5],
        ["2023-12-30", 5],
      ],
    },
  },
];

export default {
  setup(props) {
    const Month = ref("");
    const selectedMonth = ref("");
    const selectedSubject = ref("");
    const grtadeesd = ref("");
    let monthsToDates = {
      Январь: 1,
      Февраль: 2,
      Март: 3,
      Апрель: 4,
      Май: 5,
      Июнь: 6,
      Сентябрь: 9,
      Октябрь: 10,
      Ноябрь: 11,
      Декабрь: 12,
    };
    function findSubjectAndMonth(subject, month, jsonData) {
      var date = new Date(monthsToDates[month] + " 1, 2023");

      return jsonData.filter(function (data) {
        return (
          data.subject === subject &&
          monthsToDates[data.month] - 1 === date.getMonth()
        );
      });
    }

    watch(
      selectedMonth,
      (newMonth, oldMonth) => {
        grtadeesd.value = findSubjectAndMonth(
          selectedSubject.value,
          selectedMonth.value,
          jsonData
        );
      },
      { immediate: false }
    );
    watch(
      selectedSubject,
      (newMonth, oldMonth) => {
        grtadeesd.value = findSubjectAndMonth(
          selectedSubject.value,
          selectedMonth.value,
          jsonData
        );
      },
      { immediate: false }
    );
    let subjects = [
      "Введение в программирование",
      "Основы алгоритмизации",
      "Структуры данных",
      "Объектно-ориентированное программирование",
      "Компьютерные сети",
      "Базы данных",
      "Математика",
      "Английский язык",
      "Операционные системы",
      "Веб-программирование",
    ];

    let months = [
      "Январь",
      "Февраль",
      "Март",
      "Апрель",
      "Май",
      "Июнь",
      "Сентябрь",
      "Октябрь",
      "Ноябрь",
      "Декабрь",
    ];

    return {
      subjects,
      months,
      selectedMonth,
      selectedSubject,
      jsonData,
      grtadeesd,
    };
  },
};
</script>
